...
from ..builder import build_prototype_aggregation


@HEADS.register_module()
class ICPERoIHead(StandardRoIHead):
    ...
    def __init__(self,
                 support_without_mean=False,
                 prototype_aggregation: Optional[ConfigDict] = None,
                 aggregation_layer: Optional[ConfigDict] = None,
                 **kwargs) -> None:
        super().__init__(**kwargs)
        assert aggregation_layer is not None, \
            'missing config of `aggregation_layer`.'
        self.support_without_mean = support_without_mean
    ...
    def _bbox_forward_train(self, query_feats: List[Tensor],
                            ...
                            query_gt_bboxes: List[Tensor],
                            query_gt_labels: List[Tensor],
                            support_gt_labels_origin: List[Tensor]) -> Dict:
        ...
        Args:
            ...
            support_gt_labels_origin (list[Tensor]): Class indices corresponding to
        ...
        if self.support_without_mean:
            support_feat_all = self.extract_support_feats_without_mean(support_feats)
        else:
            support_feat_all = self.extract_support_feats(support_feats)
        ...
        for img_id in range(batch_size):
            ...
            support_feat = support_feat_all[img_id]
            support_feat, support_gt_labels = self.prototype_aggregation(support_feat, support_gt_labels_origin)
        ...
        if self.bbox_head.with_meta_cls_loss:
            loss_bbox['loss_meta_cls'] = []
            loss_bbox['meta_acc'] = []
            for img_id in range(batch_size):
                support_feat = support_feat_all[img_id].mean(3).mean(2)
                meta_cls_score = self.bbox_head.forward_meta_cls(support_feat)
                meta_cls_labels = torch.cat(support_gt_labels_origin)
                loss_meta_cls = self.bbox_head.loss_meta(
                    meta_cls_score, meta_cls_labels,
                    torch.ones_like(meta_cls_labels))
                for key in loss_meta_cls.keys():
                    loss_bbox[key].append(loss_meta_cls[key])
            loss_bbox['loss_meta_cls'] = torch.stack(loss_bbox['loss_meta_cls']).sum() / batch_size
            loss_bbox['meta_acc'] = torch.cat(loss_bbox['meta_acc']).mean()
    ...
    def simple_test_bboxes(
        ...
        for class_id in support_feats_dict.keys():
            ...
            if self.support_without_mean:
                support_feat = self.extract_support_feats_without_mean([support_feat])[0]
            else:
                support_feat = self.extract_support_feats([support_feat])[0]
            # support_feat = self.extract_support_feats([support_feat])[0]
            support_feat = self.prototype_aggregation.simple_test(support_feat)
